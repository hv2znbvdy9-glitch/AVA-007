...
Local defensive collection only.
Writes reports to a new Desktop folder and ZIP archive.
Does not change Firewall, Defender, Registry, users, services or tasks.
No remote execution, scanning, exploitation or counterattack functions.
#>
...
$summary = [ordered]@{
    ...
    OutputRoot = $root
    Safety = 'Local audit; report files only; no security configuration changes.'
}
Save-Section -Name '00_summary' -Value $summary

$systemInfo = Invoke-AuditSection '01_system_info' {
    $properties = @(
        'CsName', 'WindowsProductName', 'WindowsVersion', 'OsName', 'OsVersion',
        'OsBuildNumber', 'OsArchitecture', 'BiosFirmwareType', 'CsManufacturer',
        'CsModel', 'CsTotalPhysicalMemory', 'TimeZone', 'LogonServer'
    )
    Get-ComputerInfo | Select-Object -Property $properties
}
...
$defender = Invoke-AuditSection '02_defender_status' {
    if (Get-Command Get-MpComputerStatus -ErrorAction SilentlyContinue) {
        ...
        Get-MpComputerStatus | Select-Object -Property $properties
    }
    else {
...
$administrators = Invoke-AuditSection '04_local_administrators' {
    if ((Get-Command Get-LocalGroup -ErrorAction SilentlyContinue) -and
        ...
        [pscustomobject]@{ Status = 'LocalAccounts-Modul nicht verfügbar' }
    }
}
...
$users = Invoke-AuditSection '05_local_users' {
    if (Get-Command Get-LocalUser -ErrorAction SilentlyContinue) {
        ...
        [pscustomobject]@{ Status = 'Get-LocalUser nicht verfügbar' }
    }
}
...
$processes = Invoke-AuditSection '10_processes' {
    Get-Process -ErrorAction SilentlyContinue | Select-Object Id, ProcessName, Path, CPU, StartTime | Sort-Object ProcessName
}
...
$tasks = Invoke-AuditSection '12_scheduled_tasks_non_microsoft' {
    if (Get-Command Get-ScheduledTask -ErrorAction SilentlyContinue) {
        ...
        [pscustomobject]@{ Status = 'Get-ScheduledTask nicht verfügbar' }
    }
}
...
$defenderEvents = Invoke-AuditSection '16_defender_events_last_14_days' {
    try {
        ...
        } -MaxEvents 999 | Select-Object TimeCreated, ProviderName, Id, LevelDisplayName, Message
    }
    catch {
...
if ($defender -and
    $defender.PSObject.Properties.Name -contains 'RealTimeProtectionEnabled' -and
...
foreach ($profile in @($firewallProfiles)) {
    if ($profile -and $profile.PSObject.Properties.Name -contains 'Enabled' -and $profile.Enabled -eq $false) {
        Add-Finding 'MEDIUM' 'Firewall-Profil deaktiviert' "$($profile.Name) ist deaktiviert."
    }
}

foreach ($service in @($services)) {
    if ($service -and $service.Name -in @('RemoteRegistry', 'WinRM', 'TermService') -and $service.Status -eq 'Running') {
...
$htmlRows = foreach ($finding in $findings) {
    $level = ConvertTo-HtmlSafe $finding.Level
    $title = ConvertTo-HtmlSafe $finding.Title
    $detail = ConvertTo-HtmlSafe $finding.Detail
    "<tr><td>$level</td><td>$title</td><td>$detail</td></tr>"
}
if (-not $htmlRows) {
